...
namespace Simple.Abp.Account
{
    public class AccountAppService : ApplicationService, IAccountAppService
	{
		...
		public AccountAppService(IdentityUserManager userManager, IAccountEmailer accountEmailer, IAccountPhoneService phoneService, IIdentityRoleRepository roleRepository)
		{
			...
			base.LocalizationResource = typeof(AccountResource);
		}
		...
		protected virtual async Task<IdentityUser> GetUserByEmail(string email)
		{
			...
			if (user == null)
			{
				throw new BusinessException("Volo.Account:InvalidEmailAddress").WithData("Email", email);
			}
			return user;
		}
		...
		{
			var flag = await SettingProvider.IsTrueAsync("Abp.Account.IsSelfRegistrationEnabled");

			if (!flag)
			{
				throw new BusinessException("Volo.Account:SelfRegistrationDisabled");
			}
		}
